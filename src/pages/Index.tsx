...
const Index = () => {
      ...
      await new Promise<void>((resolve, reject) => {
        reader.onload = async () => {
          try {
            ...
            const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/analyze-chart`, {
              method: 'POST',
              headers: {
            ...
            try {
              ...
              const {
                data: uploadData,
                error: uploadError
              } = await supabase.storage.from('chart-images').upload(fileName, selectedImage);
  ...
  return <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Upload Zone */}
        <UploadZone onImageSelect={handleImageSelect} selectedImage={selectedImage} onClear={handleClear} />

        {/* Analyze Button */}
        {selectedImage && !result && <div className="text-center mt-8 fade-in">
            <Button onClick={handleAnalyze} disabled={isAnalyzing} size="lg" className="glass-card px-8 py-6 text-lg font-semibold hover:shadow-glow text-foreground">
              {isAnalyzing ? <>
